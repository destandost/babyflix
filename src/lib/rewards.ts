...
/** Deduct coins earned from lessons (largest subject balances first). */
export function spendEarnedCoins(amount: number): boolean {
  if (amount <= 0) return true;
  let remaining = amount;
  const balances = SUBJECTS.map((s) => ({
    id: s.id,
    coins: getSubjectProgress(s.id).coins,
  })).sort((a, b) => b.coins - a.coins);

  const total = balances.reduce((sum, b) => sum + b.coins, 0);
  if (total < amount) return false;

  for (const { id, coins } of balances) {
    if (remaining <= 0) break;
    if (coins <= 0) continue;
    const take = Math.min(coins, remaining);
    const progress = getSubjectProgress(id);
    updateSubjectProgress(id, { coins: -take });
    remaining -= take;
  }
